...
author: "Ai Zhang, Jiayi Xue, Leda, Xuhaonan Wang"
date: "11/03/2026"
...
```{r}
tocq_lda <- LDA(tocq_dtm, k = 10, control = list(seed = 1234)) # k means assume there's 10 topics need to calcuate
```

After this we can extract the per-topic-per-word probabilities, called "β" from the model:

```{r}
tocq_topics <- tidy(tocq_lda, matrix = "beta") # make a topic-word distribution
...
# find the most representative words for each topic and visualization
tocq_top_terms <- tocq_topics %>%
  group_by(topic) %>%
  top_n(10, beta) %>% # choose top 10 beta words from each topic
  ungroup() %>%
  arrange(topic, -beta) # order in topic first - then order term beta for each topic part
...
```{r}
# aim to evaluate is topic model we bulid reasonable or not?
# since the contents of the two volumes are different, the word distributions in the two volumes should also be different

tidy_tocq <- tocq %>%
  unnest_tokens(word, text) %>% # break the text into words
  anti_join(stop_words)
...
bookfreq <- tidy_tocq %>%
  mutate(booknumber = ifelse(gutenberg_id==815, "DiA1", "DiA2")) %>% # mark them in two columes
  mutate(word = str_extract(word, "[a-z']+")) %>%
  count(booknumber, word) %>%
  group_by(booknumber) %>%
  mutate(proportion = n / sum(n)) %>% # calculate the proportion of each word in each book
  select(-n) %>% 
...
ggplot(bookfreq, aes(x = DiA1, y = DiA2, color = abs(DiA1 - DiA2))) +
  ...
  scale_color_gradient(limits = c(0, 0.001), low = "darkslategray4", high = "gray75") +  # color means The frequency difference between the two books
  theme_tufte(base_family = "Helvetica") +
...
```{r}
# gamma: which topic each document belongs to

# β (beta)：topic → word
# γ (gamma)：document → topic
...
```{r}
Jane <- gutenberg_download(c(1342, 158), 
                            meta_fields = "author")
Jane
```
...
```{r}
Jane_words <- Jane %>%
  mutate(booknumber = ifelse(gutenberg_id==1342, "Pride", "Emma")) %>%
  unnest_tokens(word, text) %>%
  filter(!is.na(word)) %>%
...
```{r}
Jane_lda <- LDA(Jane_dtm, k = 8, control = list(seed = 1234)) 
Jane_topics <- tidy(Jane_lda, matrix = "beta") 

head(Jane_topics, n = 10)
```

```{r}

# find the most representative words for each topic and visualization
Jane_top_terms <- Jane_topics %>%
  group_by(topic) %>%
  top_n(10, beta) %>% # choose top 10 beta words from each topic
  ungroup() %>%
  arrange(topic, -beta) # order in topic first - then order term beta for each topic part
...
bookfreq <- tidy_Jane %>%
  mutate(booknumber = ifelse(gutenberg_id==1342, "Pride", "Emma")) %>% # mark them in two columes
  mutate(word = str_extract(word, "[a-z']+")) %>%
  count(booknumber, word) %>%
  group_by(booknumber) %>%
  mutate(proportion = n / sum(n)) %>% # calculate the proportion of each word in each book
...
ggplot(bookfreq, aes(x = Pride, y = Emma, color = abs(Pride - Emma))) +
  geom_abline(color = "black", lty = 2) +
  geom_jitter(alpha = 0.1, size = 2.5, width = 0.3, height = 0.3) +
  geom_text(aes(label = word), check_overlap = TRUE, vjust = 1.5) +
  scale_x_log10(labels = percent_format()) +
  scale_y_log10(labels = percent_format()) +
  scale_color_gradient(limits = c(0, 0.001), low = "hotpink", high = "pink") +  # color means The frequency difference between the two books
  theme_tufte(base_family = "Helvetica") +
  theme(legend.position="none", 
        strip.background = element_blank(), 
        strip.text.x = element_blank()) +
  labs(x = "Emma", y = "Pride and Prejudice") +
  coord_equal()
...
# Split into words
Jane_chapter_word <- Jane_chapter %>%
  unnest_tokens(word, text)

# Find document-word counts
Jane_word_counts <- Jane_chapter_word %>%
  anti_join(stop_words) %>%
  count(document, word, sort = TRUE) %>%
  ungroup()
...
Jane_chapters_dtm <- Jane_word_counts %>%
  cast_dtm(document, word, n)

tm::inspect(Jane_chapters_dtm)
...
```{r}
Jane_chapters_lda <- LDA(Jane_chapters_dtm, k = 2, control = list(seed = 1234))
```
...
Jane_chapters_gamma <- tidy(Jane_chapters_lda, matrix = "gamma")
Jane_chapters_gamma
...
Jane_chapters_gamma <- Jane_chapters_gamma %>%
  separate(document, c("title", "chapter"), sep = "_", convert = TRUE)

Jane_chapter_classifications <- Jane_chapters_gamma %>%
  group_by(title, chapter) %>%
  top_n(1, gamma) %>%
  ungroup()
Jane_chapter_classifications

Jane_book_topics <- Jane_chapter_classifications %>%
  count(title, topic) %>%
  group_by(title) %>%
  top_n(1, n) %>%
  ungroup() %>%
...
Jane_chapter_classifications %>%
  inner_join(Jane_book_topics, by = "topic") %>%
  filter(title != consensus)
```
# Look document-word pairs were to see which words in each documents were assigned
# to a given topic
```{r}
assignments <- augment(Jane_chapters_lda, data = Jane_chapters_dtm)
assignments

assignments <- assignments %>%
  separate(document, c("title", "chapter"), sep = "_", convert = TRUE) %>%
  inner_join(Jane_book_topics, by = c(".topic" = "topic"))
...
# load in corpus of Tocequeville text data.
corp <- corpus(Jane, text_field = "text")
# use first 10 documents for example
documents <- corp[sample(1:30000,1000)]
# take a look at the document names
print(names(documents[1:10]))
