...
```{r}
Jane <- gutenberg_download(c(1342, 1212), 
                            meta_fields = "author")
Jane
```
...
```{r}
Jane_words <- Jane %>%
  mutate(booknumber = ifelse(gutenberg_id==1342, "Pride", "Love")) %>%
  unnest_tokens(word, text) %>%
  filter(!is.na(word)) %>%
...
bookfreq <- tidy_Jane %>%
  mutate(booknumber = ifelse(gutenberg_id==1342, "Pride", "Love")) %>% # mark them in two columes
  mutate(word = str_extract(word, "[a-z']+")) %>%
  count(booknumber, word) %>%
  group_by(booknumber) %>%
  mutate(proportion = n / sum(n)) %>% # calculate the proportion of each word in each book
...
ggplot(bookfreq, aes(x = Pride, y = Love, color = abs(Pride - Love))) +
  ...
  theme(legend.position="none", 
        strip.background = element_blank(), 
        strip.text.x = element_blank()) +
  labs(x = "Love and Friendship", y = "Pride and Prejudice") +
  coord_equal()
...
pdf("assignments.pdf", width=7, height=8)
assignments %>%
  count(title, consensus, wt = count) %>%
  group_by(title) %>%
  mutate(percent = n / sum(n)) %>%
